...
int extract_tarball(const char *tar_path)
{
    ...
    if(tar_path) {
        ...
        fd = STDIN_FILENO; // Else read from STDIN
    }

    while(extract_next_file(fd));
...
int add_file_to_tarball(const char *file, int tfd, const char *tar_path)
{
    ...
    if((verbose == 1) && write_file) {
        printf("%s\n", file);
    }

    if(S_ISLNK(fs.st.st_mode) && (followSymlink == 1)) {
...
int create_tarball(const char *tar_path, int argc, char **argv)
{
    ...
    if(tar_path) {
        if(shouldUpdate == 1 || shouldAppend == 1) {
...
int create(const char *path, char ** files, int number) {
    return create_tarball(path, number, files);
}
...
int update(const char *path, char ** files, int number) {
    shouldUpdate = 1;
    return create(path, files, number);
}
